...
- QML layer: `BarWidget.qml` (bar widget) and `Panel.qml` (agenda popup), decomposed into modular QML components under `components/` (`Tokens.qml`, `HeaderBar.qml`, `HeroCard.qml`, `ScheduleGroup.qml`, `ScheduleRow.qml`, `CalendarLegend.qml`, `SetupGuide.qml`, `SetupCard.qml`, `EmptySchedule.qml`, `SettingsView.qml`, `SettingsCategoryRow.qml`, `SettingStepper.qml`, `SettingField.qml`, `SettingHint.qml`, `SettingToggle.qml`, `SettingKeyInput.qml`, `FeedCard.qml`, `SourceModeSwitcher.qml`, `ColorSpectrumPicker.qml`). These own all Qt/Quickshell UI, fetching (Quickshell.Io), and widget settings.
